...
int utf8_encode(char *buf, uint32_t code_point) {
  ...
  if (code_point <= 0x7FF) {
    buf[0] = 0b11000000 | (code_point >> 6);
    buf[1] = 0b10000000 | (code_point & 0b00111111);
    return 2;
  }

  if (code_point <= 0xFFFF) {
    buf[0] = 0b11100000 | (code_point >> 12);
    buf[1] = 0b10000000 | ((code_point >> 6) & (0b00111111));
    buf[2] = 0b10000000 | (code_point & 0b00111111);
    return 3;
  }

  buf[0] = 0b11110000 | (code_point >> 18);
  buf[1] = 0b10000000 | ((code_point >> 12) & 0b00111111);
  buf[2] = 0b10000000 | ((code_point >> 6) & 0b00111111);
  buf[3] = 0b10000000 | (code_point & 0b00111111);
  return 4;
}
...
static int char_width(uint32_t c) {
  ...
  if (in_range(range_width_0, c))
    return 0;

  static uint32_t range_width_2[] = {
