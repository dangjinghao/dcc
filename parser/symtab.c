...
astn parser_symtab_find(slist symtab, sds id) {
  ...
  slist_foreach(symtab, n) {
    if (sdscmp(parse_declaration_get_ident(n), id) == 0) {
      return n;
    }
  }
  return NULL;
...
void parser_symtab_add(parser parser, astn n) {
  ...
  if (n->declaration.storage_class == TOK_KW_EXTERN) {
    if (parser_symtab_find(&parser->symtab, parse_declaration_get_ident(n))) {
      log_debug(
          "same name symbol already exists, skip to add extern symbol: %s",
          parse_declaration_get_ident(n));
      return;
    } else {
      log_debug("add extern symbol to symtab: %s", parse_declaration_get_ident(n));
    }
  } else {
    astn existing_symbol =
        parser_symtab_find(&parser->symtab, parse_declaration_get_ident(n));
    if (existing_symbol &&
      ...
      log_panic(
          ...
          parse_declaration_get_ident(n));
               ...
               existing_symbol->declaration.storage_class == TOK_KW_EXTERN) {
      log_debug("remove extern symbol: %s", parse_declaration_get_ident(n));
      slist_remove(&parser->symtab, existing_symbol);
    }
    log_debug("add strong symbol to symtab: %s", parse_declaration_get_ident(n));
  }
