...
void build_statement_jump_return(builder b, astn n) {
  ...
  slist func_type_chain = build_type_chain_new_get_function_return_type_chain(
      b, &func->declaration.type_chain);
  ...
  if ((!n->jump_statement.expr) &&
    ...
    if (ret_val_base_type->ctype.type == TOK_KW_VOID) {
      ...
      typed_value ret_val_casted =
          build_type_convert_by_type_chain(b, ret_val, func_type_chain);
